...
Dz.LobbyPosition = {from = {x = 1012, y = 1013, z = 7},to = {x = 1026, y = 1019, z = 7}, tp = {x = 1018, y = 1017, z = 7}}
Dz.Keys = {
  [DzBronze]  = {id = 'bronze',  name = "Bronze",  itemId = 2107},
  [DzSilver]  = {id = 'silver',  name = "Silver",  itemId = 2107},
  [DzGold]    = {id = 'gold',    name = "Gold",    itemId = 2107},
  [DzSpecial] = {id = 'special', name = "Special", itemId = 2107},
}
...
function Dz.play(cid, diff, mapId)
  ...
  if state == DzStateTeam then
    ...
    elseif ret.msg == DzPreparedNotInLobby then
      ...
      return doPlayerPopupFYI(cid, (isPlayer(player) and getCreatureName(player) or "Um jogador").." nÃ£o estÃ¡ na sala de espera.")
    elseif ret.msg == DzPreparedNotHaveKey then
      return doPlayerPopupFYI(cid, (isPlayer(player) and getCreatureName(player) or "Um jogador").." nÃ£o possui chave")
    end
  else
    if not isInRange(getThingPos(cid), Dz.LobbyPosition.from, Dz.LobbyPosition.to) then
      doPlayerPopupFYI(cid, "VocÃª nÃ£o estÃ¡ na sala de espera.")
	  Dz.sendClose(cid)
	  return
    end
    local roomId = Dz.getAvailableRoom(Map)
