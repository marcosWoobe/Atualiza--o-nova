...
function adjustStatus(pk, item, health, dittoLife, conditions, isPassiva)
	...
	local ivAtk = getItemAttribute(item, "ivAtk")
	local ivDef = getItemAttribute(item, "ivDef")
	local ivSpAtk = getItemAttribute(item, "ivSpAtk")
	local ivAgi = getItemAttribute(item, "ivAgi")
	local ivHP = getItemAttribute(item, "ivHP")
	...
	if string.find(nick, "Shiny") then
		...
		setPlayerStorageValue(pk, 1001, (pokes[nick].attack + (1 * (ivAtk / 10))) * heldBonusAttack)
		setPlayerStorageValue(pk, 1002, (pokes[nick].defense + (1 * (ivDef / 10))) * heldBonusDefense)
		setPlayerStorageValue(pk, 1003, (pokes[nick].agility + (1 * (ivAgi / 10))) + heldBonusSpeed)
		setPlayerStorageValue(pk, 1004, pokes[nick].vitality * (masterLevel + getPokemonBoost(pk) + heldBonusBoost))
		setPlayerStorageValue(pk, 1005, (pokes[nick].specialattack + (1 * (ivSpAtk / 10))))
	end

    local boost = getPokemonBoost(pk) + heldBonusBoost

    -- base + ( vity * level + boost * 12.2 )
    local life = (pokes[nick].life + ((pokes[nick].vitality + (1 * (ivHP / 10))) * (masterLevel + boost)) * 12.5) * (heldBonusVitality / 100 + 1) 
...
function doEvolvePokemon(cid, item2, theevo, stone1, stone2, count)
    ...
    local c = count or 1
    if stone1 then doPlayerRemoveItem(cid, stone1, c or 1) end
    if stone2 then doPlayerRemoveItem(cid, stone2, 1) end
...
function getIvAtk(cid)
	local item = getPlayerSlotItem(getCreatureMaster(cid), 8)
	return getItemAttribute(item, "ivAtk")
end

function getIvDef(cid)
	local item = getPlayerSlotItem(getCreatureMaster(cid), 8)
	return getItemAttribute(item, "ivDef")
end

function getIvSpAtk(cid)
	local item = getPlayerSlotItem(getCreatureMaster(cid), 8)
	return getItemAttribute(item, "ivSpAttack")
end

function getIvAgi(cid)
	local item = getPlayerSlotItem(getCreatureMaster(cid), 8)
	return getItemAttribute(item, "ivAgi")
end

function getIvHP(cid)
	local item = getPlayerSlotItem(getCreatureMaster(cid), 8)
	return getItemAttribute(item, "ivHP")
end
