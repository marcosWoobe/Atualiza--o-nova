...
function sendOpcodeStatusInfo(cid, reset)
	local mid, ret = getCreatureMaster(cid), ""
	if reset and isPlayer(mid) then
		doSendPlayerExtendedOpcode(mid, 188, "reset")
	end
	if mid and isPlayer(mid) then
		for cond,storage in pairs(conds) do
			if isInArray({"Reflect", "Focus"}, cond) and getPlayerStorageValue(cid, storage) == 1 then
				ret = ret .. cond .. ";-|"
			else
				if getPlayerStorageValue(cid, storage) >= 1 then
					ret = ret .. cond .. ";".. getPlayerStorageValue(cid, storage) .."|"
				end
			end
		end
		doSendPlayerExtendedOpcode(mid, 188, ret)
	else
		return false
	end
end
...
function doBurn2(cid, cd, check, damage, eff, color, tickt, attacker)
	...
	if isPlayer(cid) and isInArea(getThingPos(cid), {x=2449,y=2611,z=8}, {x=2491,y=2654,z=8}) then return true end -- burn não pegar na dm
    if not check and getPlayerStorageValue(cid, stg) >= 1 then
       setPlayerStorageValue(cid, stg, getPlayerStorageValue(cid, stg) + cd - 1)
    else
       setPlayerStorageValue(cid, stg, getPlayerStorageValue(cid, stg) + cd)
    ...
    if isSummon(cid) and getPlayerStorageValue(cid, 212123) <= 0 then
       ...
       doItemSetAttribute(item.uid, "burneff", eff)
    end
	...
	doSendAnimatedText(getThingPosWithDebug(cid), math.floor(dano), color)
	
    addEvent(doBurn2, tickt and tickt or 1500, cid, -1, a, damage, eff, color, tickt, attacker)   
end 

function doPoison2(cid, cd, check, damage, eff)
local stg = conds["Poison"]
    if not isCreature(cid) then return true end  --is creature?
	sendOpcodeStatusInfo(cid)
    ----------
	if isPlayer(cid) and isInArea(getThingPos(cid), {x=2449,y=2611,z=8}, {x=2491,y=2654,z=8}) then return true end -- poison n pegar na dm
    if isSummon(cid) or ehMonstro(cid) and pokes[getCreatureName(cid)] then --alterado v1.6
       ...
       if isInArray({"poison", "steel"}, type) or isInArray({"poison", "steel"}, type2) then
		...
		if getCreatureName(cid) == "Zangoose" or isInArray(specialabilities["immunity"], getCreatureName(cid)) then
			return true
		end
    end
    ---------
    ...
    if isSummon(cid) and getPlayerStorageValue(cid, 212123) <= 0 then
       ...
       doItemSetAttribute(item.uid, "poisoneff", eff)
    end
...
function doSleep2(cid, cd, check, first, eff)        
local stg = conds["Sleep"]
    if not isCreature(cid) then return true end  --is creature?
	sendOpcodeStatusInfo(cid)
	if isSummon(cid) or ehMonstro(cid) and pokes[getCreatureName(cid)] then --alterado v1.6
		if isInArray(specialabilities["insomnia"], getCreatureName(cid)) then
			return true
		end
    end
    if getPlayerStorageValue(cid, stg) >= 1 and cd ~= -1 then 
	...
	if isSleeping(cid) then
		...
		if teff == 473 then
			...
			pos.x = pos.x+1
			eff = 1392
		elseif teff == 474 then
			pos.y = pos.y+1
			pos.x = pos.x+1
		end
		doSendMagicEffect(pos, teff)
	end
    ...
    if getPlayerStorageValue(cid, 3644587) == 1 then -- rollout/fire spin
        doRemoveCondition(cid, CONDITION_OUTFIT)
    end
                                      --alterado v1.6
    if isPlayer(cid) then
    ...
    addEvent(doSleep2, 1000, cid, -1, a, false, eff)
end   
...
function doBuff2(cid, cd, eff, check, buff, first, attr)
	...
	if tonumber(getPlayerStorageValue(cid, stg)) == nil or tonumber(cd) == nil then return true end
	...
	elseif buff == "Outrage" then
		doRaiseStatus(cid, 2, 2, 200, a)
    elseif buff == "Eruption" or buff == "Eerie Impulse" then
       doRaiseStatus(cid, 0, 2, 100, a)
    elseif buff == "Tailwind" then
       doRaiseStatus(cid, 0, 2, 300, a)
    elseif buff == "Rage" then
       doRaiseStatus(cid, 2, 0, 0, a)
    elseif buff == "Harden" or buff == "Charm" or buff == "Calm Mind" then
    ...
    elseif buff == "Ancient Fury" then
       ...
       if getCreatureName(cid) == "Shiny Charizard" or getCreatureName(cid) == "Elder Charizard" then 
          doRaiseStatus(cid, 2, 0, 150, a)    --atk melee     --alterado v1.6
       else
          doRaiseStatus(cid, 0, 2, 150, a)    --def
       end   
    elseif buff == "War Dog" then
       doRaiseStatus(cid, 2, 2, 0, a)
       setPlayerStorageValue(cid, 374896, 1)  --velo atk
    elseif buff == "Bulk Up" then
       doRaiseStatus(cid, 2, 2, 0, a)
    ...
    elseif isInArray({"Psychic Sight", "Heal Bell", "Future Sight", "Camouflage", "Acid Armor", "Iron Defense", "Minimize", "Withdraw"}, buff) then
		local out = outImune[buff]
		local outIron = {
		["Scizor"] = 2503,
		["Metagross"] = 1900,
		["Metang"] = 1915,
		...
		if outIron[getCreatureName(cid)] then out = outIron[getCreatureName(cid)] end
		if outAcid[getCreatureName(cid)] then out = outAcid[getCreatureName(cid)] end
       doSetCreatureOutfit(cid, {lookType = out}, -1)
       setPlayerStorageValue(cid, 9658783, 1)  
       setPlayerStorageValue(cid, 625877, out) --alterado v1.6
	   sendOpcodeStatusInfo(cid)
    elseif buff == "Bug Fighter" then
       setPlayerStorageValue(cid, 374896, 1)  --velo atk  --alterado v1.6
       doRaiseStatus(cid, 1.5, 1.5, 100, a)
       doSetCreatureOutfit(cid, {lookType = 980}, a*1000)
...
if isConditionImune(ret.id) and isWild(ret.id) and (ret.cond and not isInArray({"Burn", "Slow", "Poison", "Leech"}, ret.cond)) then
	return true
end
...
if ret.cond and not isInArray({"Poison", "Leech", "Burn"}, ret.cond) and getPlayerStorageValue(ret.id, 234247) > 0 then 
	doSendMagicEffect(getThingPosWithDebug(ret.id), 1)
	doSendAnimatedText(getThingPosWithDebug(ret.id), "IMMUNE", COLOR_TEAL)   --alterado v1.8     
	return true
end
   
if isSummon(ret.id) and getPokemonBoost(ret.id) ~= 0 and math.random(1, 100) <= math.min(50, getPokemonBoost(ret.id)) then   --sistema "pegou no boost"
	local master = getCreatureMaster(ret.id)
	local accsuc = false
	if ret.attacker then
		if isSummon(ret.attacker) then
			local atmaster = getCreatureMaster(ret.attacker)
			local atheldx = getItemAttribute(getPlayerSlotItem(atmaster, 8).uid, "xHeldItem")
			if atheldx then
			local atheldxName, atheldxTier = string.explode(atheldx, "|")[1], string.explode(atheldx, "|")[2]
			if atheldxName == "X-Accuracy" then
				local chance = heldAccuracyChance[tonumber(atheldxTier)]
				if ret.cond and math.random(1,100) <= chance then
					doSendMagicEffect(getThingPosWithDebug(ret.attacker), 47)
					doSendAnimatedText(getThingPosWithDebug(ret.attacker), "ACCURACY!", COLOR_PURPLE)
					accsuc = true
				end
			end
			end
			
			local atheldz = getItemAttribute(getPlayerSlotItem(atmaster, 8).uid, "zHeldItem") -- device
			if atheldz and not isInTourArea(getCreatureMaster(atmaster)) then
			local atheldzName, atheldzTier = string.explode(atheldz, "|")[1], string.explode(atheldz, "|")[2]
			if atheldzName == "X-Accuracy" then
				if atheldzName ~= atheldxName then
					local chance = heldAccuracyChance[tonumber(atheldzTier)]
					if ret.cond and math.random(1,100) <= chance then
						doSendMagicEffect(getThingPosWithDebug(ret.attacker), 47)
						doSendAnimatedText(getThingPosWithDebug(ret.attacker), "ACCURACY!", COLOR_PURPLE)
						accsuc = true
   ...
   if ret.cond and not isInArray({"Poison", "Leech", "Burn"}, ret.cond) and not accsuc then 
...
if isSummon(ret.id) then   --y-cure
	local master = getCreatureMaster(ret.id)
	local heldy = getItemAttribute(getPlayerSlotItem(getCreatureMaster(ret.id), 8).uid, "yHeldItem")
	...
	if heldy and not isInDuel(master) then
		...
		if heldyName == "Y-Cure" then
			...
			if (ret.cond and math.random(1, 100) <= chance) and not isInArea(getThingPos(ret.id), {x=2449,y=2611,z=8}, {x=2491,y=2654,z=8}) then
				doSendMagicEffect(getThingPosWithDebug(ret.id), 1)
				doSendAnimatedText(getThingPosWithDebug(ret.id), "CURE!", COLOR_LIGHTBLUE)   --alterado v1.8     
				return true
			end
		end
	end
	
	local heldz = getItemAttribute(getPlayerSlotItem(getCreatureMaster(ret.id), 8).uid, "zHeldItem") -- device!
...
elseif ret.cond and ret.cond == "Silence" then
	doSilence2(ret.id, isWild(ret.id) and ret.cd or math.floor(ret.cd/2), ret.eff, ret.check)
...
elseif ret.cond and ret.cond == "Burn" then
	doBurn2(ret.id, ret.cd, ret.check, ret.damage, ret.eff and ret.eff or 355, ret.color and ret.color or COLOR_BURN, ret.tickt, ret.attacker)
elseif ret.cond and ret.cond == "Poison" then
	doPoison2(ret.id, ret.cd, ret.check, ret.damage, ret.eff)
...
elseif ret.cond and ret.cond == "Sleep" then
	doSleep2(ret.id, ret.cd, ret.check, ret.first, ret.eff)
...
if not isCreature(cid) then return true end
   ...
   local skills = { "Skull Bash", "Gust", "Water Pulse", "Stick Throw", "Ground Crusher", "Overheat", "Take Down", "Gyro Ball"} --alterado v1.7
   local n = 0 
   local l = 0
...
if isWithFear(cid) and getPlayerStorageValue(cid, 3644587) >= 1 then return true end
   ...
   elseif spell and spell == "Epicenter" then
      ...
      addEvent(doDanoWithProtect, random, cid, GROUNDDAMAGE, pos[n], crusher, -min, -max, 1392)
   ...
   -- elseif spell and (spell == "Muddy Water" or spell == "Venom Motion" or spell == "Bamboo Spikes") then
      -- local arr = {
   ...
   elseif spell and (spell == "Inferno" or spell == "Fissure") then
      addEvent(sendEffWithProtect, math.random(0, 1500), cid, pos[n], eff)
      addEvent(doMoveDano2, 1000, cid, pid, element, min, max, ret, spell)
   else
...
function doMoveDano2(cid, pid, element, min, max, ret, spell)
    if isCreature(pid) and isCreature(cid) and cid ~= pid then
        ...
        if type(getPlayerStorageValue(pid, 21099)) == "string" then setPlayerStorageValue(pid, 21099, -1) end
        ...
        if isSleeping(cid) and getPlayerStorageValue(cid, 3644587) >= 1 then return true end
        if isWithFear(cid) and getPlayerStorageValue(cid, 3644587) >= 1 then return true end
        if isSummon(cid) and ((ehMonstro(pid) or (isSummon(pid) and canAttackOther(cid, pid) == "Can") or (isPlayer(pid) and canAttackOther(cid, pid) == "Can" and #getCreatureSummons(pid) <= 0)) or (isInArea(getThingPos(cid), {x=2449,y=2611,z=8}, {x=2491,y=2654,z=8}) and getCreatureMaster(cid) ~= pid and #getCreatureSummons(pid) < 1)) and pid ~= cid then
            if canAtk then     --alterado v1.6
                if ret and ret.cond then
                    if not ((ret.cond == "Burn" or ret.cond == "Poison") and isInArea(getThingPos(cid), {x=2449,y=2611,z=8}, {x=2491,y=2654,z=8})) then
                        ret.id = pid
                        ret.check = getPlayerStorageValue(pid, conds[ret.cond])
                        doCondition2(ret)
                    end
            end
            doTargetCombatHealth(cid, pid, element, -(math.abs(min)), -(math.abs(max)), 1392)
            end
...
-- function downEffect(cid, effDis)
-- if not isCreature(cid) then return true end
-- if isSleeping(cid) and getPlayerStorageValue(cid, 3644587) >= 1 then return true end
-- if isWithFear(cid) and getPlayerStorageValue(cid, 3644587) >= 1 then return true end
-- pos = getThingPos(cid)
