local attr = {"poke", "boost", "unique", "xHeldItem"}
...
if getItemNameById(item.itemid) == "Icone." then
text = text.."\n• Name: "..tsep..doCorrectString(getItemAttribute(item.uid, "ball")).." Ball\n"
else
text = text.."\n• Name: "..tsep..getItemNameById(item.itemid).." "..count
end
if getItemNameById(item.itemid) == "Icone." then
	for k,v in ipairs(attr) do
		if getItemAttribute(item.uid, v) then
		text = text .. v .. ": ".. getItemAttribute(item.uid, v) .. " | "
		end
	end
end
