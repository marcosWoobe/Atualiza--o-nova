...
if storage == -1 then
   return sendMsgToPlayer(cid, 27, "You haven't attempted to catch this pokemon yet.")
end

local t = "normal = (.-), great = (.-), super = (.-), ultra = (.-), saffari = (.-), dark = (.-), magu = (.-), sora = (.-), yume = (.-), dusk = (.-), tale = (.-), moon = (.-), net = (.-), premier = (.-), tinker = (.-), fast = (.-), heavy = (.-);"
local msg = {}
table.insert(msg, "You've spent: ")
...
if #msg == 1 then
   return doPlayerSendTextMessage(cid, 27, "You haven't attempted to catch a "..name..".")
end
...
if(param == '') then
    doPlayerSendTextMessage(cid, 27, "You must write the pokemon's name.")
    return true
end

if getPokedexNumber(doCorrectString(param)) then
   local storage = getPokemonStorageCatch(doCorrectString(param))
   local name = doCorrectPokemonName(param)
   sendBrokesMsg2(cid, name, storage)
else
   doPlayerSendTextMessage(cid, 27, "This pokemon doesn't exists.")
end
return true
end