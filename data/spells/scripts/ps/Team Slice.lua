...
if not getCreatureOutfit(cid) then return true end
...
if #getCreatureSummons(master) <= 0 then return true end
	...
	local function dmg(cid, target, min, max)
	if not isCreature(cid) or not isCreature(target) then return true end
	doTargetCombatHealth(cid, target, BUGDAMAGE, -min, -max, 255)
	end
   ...
   if #getCreatureSummons(master) >= 1 and isCreature(target) and not ehNPC(master) then
      if isCreature(cid) then
         ...
         addEvent(dmg, 500, cid, target, -minn, -maxx)
