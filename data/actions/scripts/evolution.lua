...
local bossEvo = {
['Rhydon'] = {'Rhyperior', 15070},
['Magmar'] = {'Magmortar', 15071},
['Electabuzz'] = {'Electivire', 15072},
['Dusclops'] = {'Dusknoir', 15073},
['Feebas'] = {'Milotic', 15074},
['Metang'] = {'Metagross', 15075},
['Tangela'] = {'Tangrowth', 15076},
['Magneton'] = {'Magnezone', 15077},
['Vigoroth'] = {'Slaking', 15078},
['Shelgon'] = {'Salamence', 15079},
}
...
-- PHENAC EVO --
	-- local p = phenacEvo[getCreatureName(item2.uid)]
	-- if p then
		-- if not isPlayer(getCreatureMaster(item2.uid)) or getCreatureMaster(item2.uid) ~= cid then
			...
			-- return true
		-- end
		-- if item.itemid ~= 12244 then -- ancient stone
			-- doPlayerSendCancel(cid, "You need ancient stones to evolve this pokemon.")
			-- return true
		-- end
		-- if pokes[p[1]].level > getPlayerLevel(cid) then
			-- doPlayerSendCancel(cid, "You don't have enough level.")
			-- return true
		-- end
		-- if doPlayerRemoveItem(cid, 12244, p[2] * 50) then
			-- doEvolvePokemon(cid, item2, p[1], false, false)
			-- local ball = getPlayerSlotItem(cid, 8).uid
			-- doItemSetAttribute(ball, "morta", "no")
			-- doItemSetAttribute(ball, "Icone", "yes")	
			-- doTransformItem(ball, pokeballs[(p[1]:lower())].use)
		-- else
			-- doPlayerSendCancel(cid, "You need ".. p[2] * 50 .." ancient stones to evolve this pokemon.")
			-- return true
		-- end
		-- return true
	-- end
-- BOSS EVO --
	-- local b = bossEvo[getCreatureName(item2.uid)]
	-- if b then
		-- if not isPlayer(getCreatureMaster(item2.uid)) or getCreatureMaster(item2.uid) ~= cid then
			-- doPlayerSendCancel(cid, "You can only use stones on pokemons you own.")
			-- return true
		-- end
		-- if item.itemid ~= b[2] then
			-- doPlayerSendCancel(cid, "You need a ".. getItemNameById(b[2]) .." to evolve this pokemon.")
			-- return true
		-- end
		-- if pokes[b[1]].level > getPlayerLevel(cid) then
			-- doPlayerSendCancel(cid, "You don't have enough level.")
			-- return true
		-- end
		-- if doPlayerRemoveItem(cid, b[2], 1) then
			-- doEvolvePokemon(cid, item2, b[1], false, false)
			-- local ball = getPlayerSlotItem(cid, 8).uid
			-- doItemSetAttribute(ball, "morta", "no")
			-- doItemSetAttribute(ball, "Icone", "yes")	
			-- doTransformItem(ball, pokeballs[(b[1]:lower())].use)
		-- else
			-- doPlayerSendCancel(cid, "You need a ".. getItemNameById(b[2]) .." to evolve this pokemon.")
			-- return true
		-- end
...
-- end
	...
	doEvolvePokemon(cid, item2, evo, stnid, stnid2, count)
	local ball = getPlayerSlotItem(cid, 8).uid
	doItemEraseAttribute(ball, "boost")	
	doItemSetAttribute(ball, "morta", "no")
	doItemSetAttribute(ball, "Icone", "yes")	
