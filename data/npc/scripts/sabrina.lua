...
local pokemons = {
    {name = "Shiny Alakazam", optionalLevel = 1, sex = SEX_MALE, nick = "", ball = "poke"},
    {name = "Shiny Hypno", optionalLevel = 1, sex = SEX_MALE, nick = "", ball = "poke"},
    {name = "Shiny Mr. Mime", optionalLevel = 1, sex = SEX_FEMALE, nick = "", ball = "poke"},
    {name = "Shiny Jynx", optionalLevel = 1, sex = SEX_FEMALE, nick = "", ball = "poke"},
    {name = "Porygon2", optionalLevel = 1, sex = SEX_FEMALE, nick = "", ball = "poke"},
    {name = "Shiny Espeon", optionalLevel = 1, sex = SEX_FEMALE, nick = "", ball = "poke"},
}
...
local function doSummonGymPokemon(npc)
    ...
    doSetGym(summon, 1)
    if #getCreatureSummons(focus) >= 1 then
        doSetMonsterGym(summon, summons)
    end
    addEvent(adjustWildPoke, 100, summon, it.optionalLevel)
...
function onCreatureSay(cid, type, msg)
    ...
    if isConfirmMsg(msg) and conv == 2 and focus == cid then
        if #getCreatureSummons(cid) >= 1 then
            selfSay("Please return your pokemon before the battle.!")
            return true
        end
        challenger = focus
            ...
            if not isCreature(getCreatureTarget(getThis())) then
                if #getCreatureSummons(challenger) >= 1 then
                    if getCreatureOutfit(getCreatureSummons(challenger)[1]).lookType ~= 2 then --alterado v1.6
                        doSetAttackGym(getCreatureSummons(getThis())[1], getCreatureSummons(challenger)[1])
                    end
                else
