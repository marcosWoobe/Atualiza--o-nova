...
local talkUser = NPCHANDLER_CONVBEHAVIOR == CONVERSATION_DEFAULT and 0 or cid
        ...
        if (msgcontains(msg, 'shiny') or msgcontains(msg, 'Shiny')) then    --alterado v1.7
           if getPlayerClanName(cid) ~= 'Ironhard' then    --alterado v1.7
              selfSay("You aren't of the clan Ironhard! Get out of here!", cid)
              return true
           else   
              selfSay("So you want to transform your Lucario in a Shiny Lucario? It will cost 500k!", cid)
        ...
        elseif (msgcontains(msg, 'yes') or msgcontains(msg, 'Yes')) and talkState[talkUser] == 4 then
	       ...
	       if getItemAttribute(pb, "poke") ~= "Lucario" then
	          selfSay("Put a Lucario's pokeball atleast +20 in main slot!", cid)
              talkState[talkUser] = 0
              return true
           end
              
           if not getItemAttribute(pb, "boost") or getItemAttribute(pb, "boost") < 20 then
              selfSay("Sorry, your Lucario is not boosted +20!", cid)
              talkState[talkUser] = 0
              return true
           end
           ...
           if doPlayerRemoveMoney(cid, 500000) == true then
              ...
              doItemSetAttribute(pb, "poke", "Shiny Lucario")
		      doItemSetAttribute(pb, "description", "Contains a Shiny Lucario.")
		      doItemEraseAttribute(pb, "boost")
		      doTransformItem(getPlayerSlotItem(cid, 7).uid, fotos["Shiny Lucario"])
