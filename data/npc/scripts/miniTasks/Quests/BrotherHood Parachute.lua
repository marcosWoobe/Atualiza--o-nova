...
elseif((msgcontains(string.lower(msg), 'help')) and (getDistanceToCreature(cid) <= 3) and talkState[talkUser] == 1) then
	...
	if storBro == -1 then -- nao participa do club
	   ...
	   selfSay("Posso mudar seu rank no club por 10 mil dolares. Os nives são 'Easy', 'Medio', 'Hard', 'Expert'. Isso vai resetar seus contratos atuais.")
	end
...
elseif((msgcontains(string.lower(msg), 'participar')) and (getDistanceToCreature(cid) <= 3) and talkState[talkUser] == 2) then
	...
	selfSay("Nos procuramos foras da lei e para se tornar um membro MESTRE preciso que mate 1000 deles. Volte quando estiver pronto.")
	setPlayerStorageValue(cid, storages.BrotherHoodMember, 1)
	setPlayerStorageValue(cid, storages.BrotherHoodMemberOUTFITDUELIST, 1)
