...
local getNpcTaskName = getPlayerStorageValue(cid, storages.miniQuests.storNpcTaskName)
local pokeTask1 = getPlayerStorageValue(cid, storages.miniQuests.storPokeNameTask1)
local pokeCountTask1 = tonumber(getPlayerStorageValue(cid, storages.miniQuests.storPokeCountTask1))
local minhaHora = getPlayerStorageValue(cid, storages.miniQuests.storDayTask1)
	
if tonumber(minhaHora) ~= tonumber(hora) then 
	setPlayerStorageValue(cid, storages.miniQuests.storNpcTaskName, -1)
	setPlayerStorageValue(cid, storages.miniQuests.storPokeNameTask1, -1)
	setPlayerStorageValue(cid, storages.miniQuests.storPokeCountTask1, -1)
...
if ((pokeTask1 == strinSay1 or pokeTask1 == strinSay2) and getNpcTaskName == getCreatureName(getThis())) and msgcontains(string.lower(msg), 'hi') then	
   ...
   if pokeCountTask1 <= 0 then
	   ...
	   setPlayerStorageValue(cid, storages.miniQuests.storPokeNameTask1, "amanha")
...
count = 0
	...
	if((msgcontains(string.lower(msg), 'hi') or  msgcontains(string.lower(msg), 'help')) and (getDistanceToCreature(cid) <= 3)) then
		...
		selfSay('Olá! Minhas tasks são do nivel Facil. Você pode matar ' .. stringToSay1 .. " ou " .. stringToSay2 .. ".")
	...
	elseif msgcontains(string.lower(msg), string.lower(strinSay1)) or msgcontains(string.lower(msg), string.lower(strinSay2)) and getDistanceToCreature(cid) <= 3 and talkState[talkUser] == 1 then	
		...
		if msgcontains(string.lower(msg), string.lower(strinSay1)) then
			setPlayerStorageValue(cid, storages.miniQuests.storNpcTaskName, getCreatureName(getThis()))
			setPlayerStorageValue(cid, storages.miniQuests.storPokeNameTask1, opcoesDiarias[1][1][1])
			setPlayerStorageValue(cid, storages.miniQuests.storPokeCountTask1, opcoesDiarias[1][1][2])
			setPlayerStorageValue(cid, storages.miniQuests.storDayTask1, hora)
			pokeCounts = opcoesDiarias[1][1][2]
		else
			setPlayerStorageValue(cid, storages.miniQuests.storNpcTaskName, getCreatureName(getThis()))
			setPlayerStorageValue(cid, storages.miniQuests.storPokeNameTask1, opcoesDiarias[2][1][1])
			setPlayerStorageValue(cid, storages.miniQuests.storPokeCountTask1, opcoesDiarias[2][1][2])
			setPlayerStorageValue(cid, storages.miniQuests.storDayTask1, hora)
