...
void LuaScriptInterface::registerFunctions()
{
    ...
    lua_register(m_luaState, "doMoveCreatureToPos", LuaScriptInterface::luaDoMoveCreatureToPos);

    //canMoveToPos(cid, pos)
    lua_register(m_luaState, "canMoveToPos", LuaScriptInterface::luaCanMoveToPos);
    ...
    //getPlayerCap(cid)
    lua_register(m_luaState, "getPlayerCap", LuaScriptInterface::luagetPlayerCap);

    //setPlayerCap(cid, newCap)
    lua_register(m_luaState, "setPlayerCap", LuaScriptInterface::luasetPlayerCap);

    //sendReloginWindow(cid)
    lua_register(m_luaState, "sendReloginWindow", LuaScriptInterface::luasendReloginWindow);

    //getCreatureNick(cid)
    lua_register(m_luaState, "getCreatureNick", LuaScriptInterface::luaGetCreatureNick);

    //openChannelDialog(cid)
    lua_register(m_luaState, "openChannelDialog", LuaScriptInterface::luaOpenChannelDialog);

    //doCreatureSetShieldType(cid, shield)
    lua_register(m_luaState, "doCreatureSetShieldType", LuaScriptInterface::luaDoCreatureSetShieldType);
	...
	//doCreatureAddHealth(attacker, cid, health[, hitEffect[, hitColor[, force]]])
	lua_register(m_luaState, "doCreatureAddHealth", LuaScriptInterface::luaDoCreatureAddHealth);

	//doAddConditon(pos, area)
	lua_register(m_luaState, "getCreaturesInArea", LuaScriptInterface::luaGetCreaturesInArea);

	//doCreatureRemoveHealth(attacker, defenser, value)
	lua_register(m_luaState, "doCreatureRemoveHealth", LuaScriptInterface::luadoCreatureRemoveHealth);
	...
	//doCreateMonsterNick(master, pokeName, pokeNick, pos, useMaster)
	lua_register(m_luaState, "doCreateMonsterNick", LuaScriptInterface::luaDoCreateMonsterNick);

	//doKillCreature(cid[, listKillers])
	lua_register(m_luaState, "doKillCreature", LuaScriptInterface::luadoKillCreature);

	//doKillWildPoke(matou, morreu)
	lua_register(m_luaState, "doKillWildPoke", LuaScriptInterface::luadoKillWildPokemon);

	//doPokemonSetNoTarget(cid)
	lua_register(m_luaState, "doPokemonSetNoTarget", LuaScriptInterface::luadoPokemonSetNoTarget);

	//setCreatureName(cid, name, description)
     lua_register(m_luaState, "setCreatureName", LuaScriptInterface::luaSetCreatureName);

     //getNPCXMLNAME(nid)
     lua_register(m_luaState, "getNPCXMLNAME", LuaScriptInterface::luagetNPCXMLNAME);
	...
	//doRemoveSummon(cid, target)
	lua_register(m_luaState, "doRemoveSummon", LuaScriptInterface::luaDoRemoveSummon);
	...
	//doMosterSetAttackSpeed(cid, speed)
	lua_register(m_luaState, "doMosterSetAttackSpeed", LuaScriptInterface::luadoMosterSetAttackSpeed);
	...
	//isPartyEnabledExp(cid)
	lua_register(m_luaState, "isPartyEnabledExp", LuaScriptInterface::luaIsPartyEnabledExp);

	//doSendMsgToPartyChannel(cid, msg)
	lua_register(m_luaState, "doSendMsgToPartyChannel", LuaScriptInterface::luaDoSendMsgToPartyChannel);
...
int32_t LuaScriptInterface::luaDoMoveCreatureToPos(lua_State* L)
{
	//doMoveCreatureToPos(cid, pos)
	PositionEx pos;
	popPosition(L, pos);
	...
	if(creature)
	   g_game.steerCreature(creature, pos, 5);


	return 1;
}

int32_t LuaScriptInterface::luadoMosterSetAttackSpeed(lua_State* L)
{
	//doMosterSetAttackSpeed(cid, speed)
	...
	if(m)
	   m->setAttackSpeed(speed);


	return 1;
}

int32_t LuaScriptInterface::luaCanMoveToPos(lua_State* L)
{
	//canMoveToPos(cid, pos) mexer aqui depois para pokemons fantasmas
	PositionEx pos;
	popPosition(L, pos);
	uint32_t uid = popNumber(L);
	ScriptEnviroment* env = getEnv();
	...
	if(creature)
    {
    	...
    	if(g_game.getPathToEx(creature, pos, dirList, fpp))
        {
    		lua_pushboolean(L, true);
    		return true;
        }
...
int32_t LuaScriptInterface::luadoKillCreature(lua_State* L)
{
	//doKillCreature(cid[, listKillers])
	ScriptEnviroment* env = getEnv();
...
int32_t LuaScriptInterface::luadoKillWildPokemon(lua_State* L)
{
	//doKillWildPokemon(matou, morreu)
	ScriptEnviroment* env = getEnv();
	Creature* marreu = env->getCreatureByUID(popNumber(L));
	Creature* matou = env->getCreatureByUID(popNumber(L));
	 if(marreu && matou){
       CreatureEventList s = marreu->getCreatureEvents(CREATURE_EVENT_MATOU);
		for(CreatureEventList::iterator it = s.begin(); it != s.end(); ++it)
		{
			(*it)->executeMatou(matou, marreu);
		}
     }
...
int32_t LuaScriptInterface::luadoPokemonSetNoTarget(lua_State* L)
{
	//doPokemonSetNoTarget(cid)
	ScriptEnviroment* env = getEnv();
	Creature* creature = env->getCreatureByUID(popNumber(L));
	Monster* monster = creature->getMonster();
	 if(monster){
       monster->setFollowCreature(NULL);
     }else{
       std::cout << "doPokemonSetNoTarget(cid): Erro. Criatura n\E3o encontrada." << std::endl;
           }
...
{
	//doCreateMonsterNick(master, pokeName, pokeNick, pos, useMaster)
	bool useMaster = false, pass = false;

	if(lua_gettop(L) > 4)
		useMaster = popNumber(L);

	PositionEx pos;
	popPosition(L, pos);
	std::string pokeNick = popString(L);
	std::string pokeName = popString(L);
	uint32_t uid = popNumber(L);
	...
	Monster* monster = Monster::createMonsterNick(pokeName.c_str(), pokeNick.c_str());
	if(monster == 0) {
         errorEx("Monster with name '" + pokeName + "' not found");
	  	 lua_pushboolean(L, false);
         return 1;
    }
	Creature* creature_stor = (Creature*) monster;
	creature_stor->setStorage(510, pokeName);
	if(!monster)
	{
			errorEx("Monster with name '" + pokeName + "' not found");
	...
	{
		lua_pushstring(L, "Nao");
		return 1;
	}

	if(useMaster){
	  if(pass)
	    monster->convinceCreature(creature);
	}
...
int32_t LuaScriptInterface::luaDoCreateMonster(lua_State* L)
{
	...
	Creature* creature_stor = (Creature*) monster;
	creature_stor->setStorage(510, name);
	if(!monster)
	{
		if(displayError)
			errorEx("Monster with name '" + name + "' not found");
...
int32_t LuaScriptInterface::luagetPlayerCap(lua_State* L)
{
	//getPlayerCap(cid)
	ScriptEnviroment* env = getEnv();
	if(Player* player = env->getPlayerByUID(popNumber(L)))
...
int32_t LuaScriptInterface::luasetPlayerCap(lua_State* L)
{
	//setPlayerCap(cid, newCap)
	double newCap = popNumber(L);
	ScriptEnviroment* env = getEnv();
	if(Player* player = env->getPlayerByUID(popNumber(L)))
...
int32_t LuaScriptInterface::luaGetCreaturesInArea(lua_State* L)
{
	//getCreaturesInArea(pos, area)
	uint32_t rowsArea = 0;
	...
	Area* area = new Area(listArea, rowsArea);
	std::list<Tile*> tileList = area->getList(pos);
	...
	for(std::list<Tile*>::iterator it = tileList.begin(); it != tileList.end(); ++it)
	{
        ...
        {
            lua_pushnumber(L, i);
            lua_pushnumber(L, env->addThing(*cit));
            pushTable(L);
            i++;
        }
	}
...
int32_t LuaScriptInterface::luadoCreatureRemoveHealth(lua_State* L)
{
	//doCreatureRemoveHealth(attacker, cid, value)
	...
	Creature* defenser = env->getCreatureByUID(popNumber(L));
	Creature* attacker = env->getCreatureByUID(popNumber(L));
	...
	if(defenser != NULL)
	{
         if(attacker != NULL)
         {
            ...
            attacker, defenser, healthChange, hitEffect, hitColor, false);
...
{
	//doCreatureAddHealth(uid, health[, hitEffect[, hitColor[, attacker]]])
	int32_t params = lua_gettop(L);
	ScriptEnviroment* env = getEnv();

	uint32_t attacker = 0;
	if(params > 4)
		attacker = popNumber(L);
	...
	int32_t healthChange = popNumber(L);
	Creature* creature = env->getCreatureByUID(popNumber(L));
	Creature* attk = env->getCreatureByUID(attacker);
	if(creature != NULL)
	{
         if(attk != NULL)
         {
          g_game.combatChangeHealth(healthChange < 1 ? COMBAT_UNDEFINEDDAMAGE : COMBAT_HEALING,
		  attk, creature, healthChange, hitEffect, hitColor, false);
          return 1;
         }

          g_game.combatChangeHealth(healthChange < 1 ? COMBAT_UNDEFINEDDAMAGE : COMBAT_HEALING,
		  NULL, creature, healthChange, hitEffect, hitColor, false);
	      lua_pushboolean(L, true);
...
int32_t LuaScriptInterface::luaDoPlayerAddItem(lua_State* L)
{
	...
	{
		if(it.stackable)
			itemCount = (int32_t)std::ceil((float)count / 10000);
	...
	while(itemCount > 0)
	{
		...
		ReturnValue ret = g_game.internalPlayerAddItem(NULL, player, newItem, (slots_t)slot);
		if(ret != RET_NOERROR)
		{
			delete newItem;
			lua_pushboolean(L, false);
...
int32_t LuaScriptInterface::luaDoPlayerAddItemEx(lua_State* L)
{
	...
	if(item->getParent() == VirtualCylinder::virtualCylinder)
		lua_pushnumber(L, g_game.internalPlayerAddItem(NULL, player, item, (slots_t)slot));
	else
		lua_pushboolean(L, false);
...
int32_t LuaScriptInterface::luaSetCreatureName(lua_State* L)
{
    ...
    ScriptEnviroment* env = getEnv();
    Creature* creature;
    creature = env->getCreatureByUID(popNumber(L));
    if(creature){
...
int32_t LuaScriptInterface::luagetNPCXMLNAME(lua_State* L)
{
	//getNPCXMLNAME(nid)
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
        if(Npc* npc = creature->getNpc()){
		  lua_pushstring(L, npc->fileNAMETOGET.c_str());
       }
	}
	else
	{
...
int32_t LuaScriptInterface::luaDoSummonMonster(lua_State* L) // alterado para o sistema de shiny nome e apelidos
{
	//doSummonMonster(cid, name, nick)
	std::string nick = "";
	if(lua_gettop(L) > 2)
		nick = popString(L);
	...
	bool flag = false;
	if(nick == ""){
       if(g_game.placeSummon(creature, name))
            flag = true;
    }else{
       if(g_game.placeSummon(creature, name, nick))
            flag = true;
    }
...
int32_t LuaScriptInterface::luaDoRemoveSummon(lua_State* L)
{
	//doRemoveSummon(cid, target)
	uint32_t cid = popNumber(L);
	...
	creature->getMonster()->setFollowCreature(target, true);
	lua_pushboolean(L, true);
	return 1;
}
...
{
 //openChannelDialog(cid, channels)
 ScriptEnviroment* env = getEnv();
 if(Player* player = env->getPlayerByUID(popNumber(L))){
...
int32_t LuaScriptInterface::luasendReloginWindow(lua_State* L)
{
 //sendReloginWindow(cid)
 ScriptEnviroment* env = getEnv();
 if(Player* player = env->getPlayerByUID(popNumber(L))){
    player->sendReLoginWindow();
    return 1;
 }else
    return 0;
}
...
int32_t LuaScriptInterface::luaGetCreatureNick(lua_State* L)
{
	//getCreatureNick(cid)
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L))){
        ...
        if(Player* player = creature->getPlayer()){
	    player->getName();
          if(creature->getStorage(503, strValue) && strValue != "-1")
             lua_pushstring(L, strValue.c_str());
        }else
...
int32_t LuaScriptInterface::luaGetCreatureName(lua_State* L)
{
	...
	if(Creature* creature = env->getCreatureByUID(popNumber(L))){
       Monster* m = creature->getMonster();
 	   std::string strValue;
        if(creature->getStorage(510, strValue) && strValue != "-1"){
           nome = strValue;
           if(getNick)
             nome = m->mType->realName;
        }else
           nome = creature->getName();
...
int32_t LuaScriptInterface::luaDoSendMsgToPartyChannel(lua_State* L)
{
	//doSendMsgToPartyChannel(cid, msg)
	std::string loot = popString(L);
	uint32_t cid = popNumber(L);
...
int32_t LuaScriptInterface::luaIsPartyEnabledExp(lua_State* L)
{
	//isPartyEnabledExp(cid)
	uint32_t cid = popNumber(L);
	...
	{
		if(Party* party = player->getParty())
			lua_pushboolean(L, party->isSharedExperienceEnabled());
	}
	else
	{
		errorEx(getError(LUA_ERROR_PLAYER_NOT_FOUND));
