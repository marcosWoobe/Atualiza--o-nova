...
bool argumentsHandler(StringVec args)
{
	...
	for(StringVec::iterator it = args.begin(); it != args.end(); ++it)
	{
		...
		if((*it) == "--version")
		{
			std::cout << "DXP Server. Compilado by Taiger" << std::endl;
			return false;
		}
...
ServiceManager* services)
{
	...
	std::cout << "EterniumOT by Raafa" << std::endl;
	...
	std::cout << ">> Carregando config (" << g_config.getString(ConfigManager::CONFIG_FILE) << ")" << std::endl;
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Carregando config");
	#endif
	if(!g_config.load())
	...
	std::cout << ">> Carregando RSA key" << std::endl;
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Carregando RSA Key");
	#endif
	...
	std::cout << ">> Iniciando conexion con bas de datos" << std::endl;
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	...
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Carregando items");
	#endif
	...
	std::cout << ">> Carregando groups" << std::endl;
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Carregando groups");
	#endif
	if(!Groups::getInstance()->loadFromXml())
		startupErrorMessage("Não foi possível abrir o groups.xml!");

	std::cout << ">> Carregando vocation" << std::endl;
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Carregando vocation");
	#endif
	if(!Vocations::getInstance()->loadFromXml())
		startupErrorMessage("Não foi possível carregar as vocation!");

	std::cout << ">> Carregando scripts" << std::endl;
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Carregando script systems");
	#endif
	if(!ScriptingManager::getInstance()->load())
		startupErrorMessage("");

	std::cout << ">> Carregando chat channels" << std::endl;
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Carregando chat channels");
	#endif
	...
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Carregando outfits");
	#endif
	if(!Outfits::getInstance()->loadFromXml())
		startupErrorMessage("Unable to load outfits!");

	std::cout << ">> Carregando pokémons" << std::endl;
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Carregando pokémons");
	#endif
	if(!g_monsters.loadFromXml())
	{
		#if defined(WINDOWS) && !defined(__CONSOLE__)
		if(MessageBox(GUI::getInstance()->m_mainWindow, "Unable to load pokémons! Continue?", "Monsters", MB_YESNO) == IDNO)
		#else
		std::cout << "Unable to load pokémons! Continue? (y/N)" << std::endl;
		char buffer = getchar();
		if(buffer == 10 || (buffer != 121 && buffer != 89))
		#endif
			startupErrorMessage("Unable to load pokémons!");
	}

	std::cout << ">> Carregando mods..." << std::endl;
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Carregando mods...");
	#endif
	if(!ScriptingManager::getInstance()->loadMods())
		startupErrorMessage("Unable to load mods!");

	std::cout << ">> Carregando o mapa e respawns..." << std::endl;
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Carregando o mapa e respawns..");
	#endif
	...
	// std::cout << ">> Estado del servidor: PvP" << std::endl;
	

	std::cout << ">> Iniciando os serviços..." << std::endl;
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Iniciando serviços..");
	#endif
	...
	std::cout << std::endl << ">> Carregado completo, servidor ONLINE!" << std::endl;
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Carregado completo, servidor ONLINE!");
	#endif
	g_game.setGameState(GAME_STATE_NORMAL);
...
LRESULT CALLBACK WindowProcedure(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(message)
	{
		...
		case WM_DESTROY:
		{
			if(MessageBox(hwnd, "Você tem certeza que deseja fechar o servidor?", "Shutdown", MB_YESNO) == IDYES)
			{
				Shell_NotifyIcon(NIM_DELETE, &NID);
...
int32_t WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszArgument, int32_t WindowStyle)
{
	...
	GUI::getInstance()->m_mainWindow = CreateWindowEx(0, "darkXpoke", "darkXpoke 1.6", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 640, 450, HWND_DESKTOP, NULL, hInstance, NULL);
