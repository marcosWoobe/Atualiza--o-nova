...
void ProtocolGame::parsePacket(NetworkMessage &msg)
{
    ...
    if(player->isTvWatching() && recvbyte == 0x82){
       switch(recvbyte)
		{
...
void ProtocolGame::parseUseItem(NetworkMessage& msg)
{
	...
	//g_game.playerUseItem(player->getID(), pos, stackpos, index, spriteId, isHotkey);
}
...
void ProtocolGame::protocolSendDuelChannels(int channels)
{
	NetworkMessage_ptr msg = getOutputBuffer();
	if(msg)
	{
		...
		msg->AddByte(0xAB);
		ChannelList list = g_chat.getChannelList(player);
		if(channels == 0) // janelas de escolha de quantidade de players a duelar
		   msg->AddByte((list.size() - (list.size() -3)));
		else if (channels == 1) // janelas de escolha de quantidade de pokemons para duelar
		   msg->AddByte((list.size() - (list.size() -6)));
		...
		for(ChannelList::iterator it = list.begin(); it != list.end(); ++it)
		{
			...
			{ 
			    if(channels == 0){
				  if((channel->getId() >= 8 && channel->getId() <= 10)){
				     msg->AddU16(channel->getId());
				     msg->AddString(channel->getName());
				  }
				}else if(channels == 1){
				  if((channel->getId() >= 11 && channel->getId() <= 16)){
				     msg->AddU16(channel->getId());
				     msg->AddString(channel->getName());
				  }
				}
