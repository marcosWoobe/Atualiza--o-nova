...
Creature* Game::getCreatureByName(std::string s)
{
	...
	for(AutoList<Creature>::iterator it = autoList.begin(); it != autoList.end(); ++it)
	{
		...
		if(asLowerCaseString(it->second->getName()) == s)
			return it->second;
        else if(Monster* m = it->second->getMonster()){
                if(asLowerCaseString(m->getRealName()) == s)
                   return it->second;
        }
        }
	}
...
ReturnValue Game::placeSummon(Creature* creature, const std::string& name, const std::string& nick)
{
	Monster* monster = Monster::createMonsterNick(name, nick);
    monster->setStorage(510, name);
    monster->setStorage(503, nick);
...
bool Game::playerLookInTrade(uint32_t playerId, bool lookAtCounterOffer, int32_t index)
{
	...
	{
		if(!(*it)->executeLook(player, tradeItem, player->getPosition(), 0, 0))
			deny = true;
	}
...
bool Game::playerSay(uint32_t playerId, uint16_t channelId, SpeakClasses type, const std::string& receiver, const std::string& text)
{
   ...
   if(player->isTvWatching() && (channelId < 100 || channelId > 10000))
      return false;
