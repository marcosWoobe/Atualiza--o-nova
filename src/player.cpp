...
void Player::sendCancelMessage(ReturnValue message) const
{
	switch(message)
	{
		...
		case RET_NOTENOUGHCAPACITY:
			sendCancel("Voce so pode carregar 6 pokemons.");
			break;

		case RET_CONTAINERNOTENOUGHROOM:
			sendCancel("Voce nao pode colocar mais objetos neste recipiente.");
			break;
		...
		case RET_CANNOTPICKUP:
			sendCancel("Voce nao pode pegar o objeto.");
			break;
		...
		case RET_THEREISNOWAY:
			sendCancel("Nao ha nenhuma maneira.");
			break;
...
bool Player::onKilledCreature(Creature* target, uint32_t& flags)
{
	...
	pzLocked = true;
	if((condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_INFIGHT, g_config.getNumber(ConfigManager::WHITE_SKULL_TIME))))
		addCondition(condition);
